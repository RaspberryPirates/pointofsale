...
public class AuthorizeDotNetProcessor implements CardProcessor {
	...
	public void chargeAmount(PosTransaction transaction) throws Exception {
		...
		if (result.isApproved()) {
			...
			populateResult(result, transaction, creditCard);
		}
//		else if (result.isDeclined()) {
//			throw new Exception(Messages.getString("Authori8zeDotNetProcessor.2") + result.getResponseReasonCodes().get(0).getReasonText()); //$NON-NLS-1$
//		}
//		else {
//			throw new Exception(Messages.getString("AuthorizeDotNetProcessor.3") + result.getResponseReasonCodes().get(0).getReasonText()); //$NON-NLS-1$
//		}
	...
	private void populateResult(Result<Transaction> result, PosTransaction transaction) {
		populateResult(result, transaction, result.getTarget().getCreditCard());
	}
	
	private void populateResult(Result<Transaction> result, PosTransaction transaction, CreditCard card) {
		transaction.setCardTransactionId(UUID.randomUUID().toString());
		transaction.setCardAuthCode(UUID.randomUUID().toString().substring(0,7));
